...
RED_PERU = "10.192.32.0/19"
SUBREDES_REQUERIDAS = 8
PREFIJO_SEDES = 22
PREFIJO_WAN = 30
SALIDA = Path("SEDE-PRINCIPAL-WAN-IA.xlsx")
...
def validate_contiguous(parent: ipaddress.IPv4Network, subnets: list[ipaddress.IPv4Network], prefix: int) -> None:
    assert len(subnets) == SUBREDES_REQUERIDAS, "No salieron 8 subredes"
    assert all(net.prefixlen == prefix for net in subnets), f"Alguna subred no es /{prefix}"
    ...
    assert subnets[-1].broadcast_address == parent.broadcast_address, "La ultima subred no cubre el final"


def build_sheet() -> str:
    peru = ipaddress.ip_network(RED_PERU)
    sedes_subnets = list(peru.subnets(new_prefix=PREFIJO_SEDES))
    validate_contiguous(peru, sedes_subnets, PREFIJO_SEDES)

    nombres = ["Reservado", "WAN", "Lima", "La Libertad", "Ica", "Huanuco", "Puno", "Reserva"]

    wan_parent = sedes_subnets[1]
    wan_links = list(wan_parent.subnets(new_prefix=PREFIJO_WAN))[:4]
    assert len(wan_links) == 4
    assert all(net.subnet_of(wan_parent) for net in wan_links)
    enlaces = ["Lima - Ica", "Lima - La Libertad", "Lima - Huanuco", "Lima - Puno"]

    rows: list[str] = [
        row_xml(1, {1: ("Nombre de la Sede:", 1), 2: ("Casa Matriz Perú", 2)}),
        row_xml(2, {1: ("Direccion IP (subred):", 1), 2: (RED_PERU, 2)}),
        row_xml(3, {1: ("N° de subredes", 1), 2: (SUBREDES_REQUERIDAS, 2)}),
        row_xml(7, {3: ("Dirección IP padre en Binario", 3)}),
    ]
    ...
    headers = {
        ...
        13: "Nombre de la sede",
    }
    rows.append(row_xml(8, {c: (v, 4 if c not in (7, 10, 11, 12) else 5) for c, v in headers.items()}))

    for i, net in enumerate(sedes_subnets):
    ...
    rows.append(row_xml(19, {7: ("TABLA WAN", 4), 8: ("Direccion de Red", 5), 9: ("Mascara", 4), 10: ("Primer Host", 5), 11: ("Segundo Host", 5), 12: ("Broadcast", 4)}))
    for i, (enlace, net) in enumerate(zip(enlaces, wan_links), start=20):
        rows.append(
            row_xml(
                i,
                {
                    7: (enlace, 7),
                    8: (str(net.network_address), 6),
                    9: (str(net.netmask), 6),
                    10: (str(net.network_address + 1), 6),
                    11: (str(net.network_address + 2), 6),
                    12: (str(net.broadcast_address), 6),
                },
            )
        )
...
<worksheet xmlns="http://schemas.openxmlformats.org/spreadsheetml/2006/main"
 ...
 <mergeCells count="2"><mergeCell ref="C7:F7"/><mergeCell ref="B1:D1"/></mergeCells>
</worksheet>'''
...
<styleSheet xmlns="http://schemas.openxmlformats.org/spreadsheetml/2006/main">
 ...
 <cellXfs count="8">
  ...
  <xf numFmtId="0" fontId="1" fillId="5" borderId="1" xfId="0" applyFill="1" applyFont="1" applyBorder="1"/>
 </cellXfs>
...
<workbook xmlns="http://schemas.openxmlformats.org/spreadsheetml/2006/main"
 xmlns:r="http://schemas.openxmlformats.org/officeDocument/2006/relationships">
 <sheets><sheet name="Sede Principal" sheetId="1" r:id="rId1"/></sheets>
</workbook>''',
...
<Relationships xmlns="http://schemas.openxmlformats.org/package/2006/relationships">
 ...
 <Relationship Id="rId2" Type="http://schemas.openxmlformats.org/officeDocument/2006/relationships/styles" Target="styles.xml"/>
    ...
    peru = ipaddress.ip_network(RED_PERU)
    sedes = list(peru.subnets(new_prefix=PREFIJO_SEDES))
    wan = list(sedes[1].subnets(new_prefix=PREFIJO_WAN))[:4]
    print(f"OK: {SALIDA.resolve()}")
    print(f"Red Peru: {peru}")
    print(f"Subredes de sede: {len(sedes)} con prefijo /{PREFIJO_SEDES}")
    print(f"Bloque WAN: {sedes[1]}")
    print(f"Enlaces WAN usados: {len(wan)} con prefijo /{PREFIJO_WAN}")
    print("Validacion: sedes sin huecos/solapamientos y enlaces WAN dentro del bloque WAN.")
