...
RED_LIMA = "10.192.40.0/22"
CRECIMIENTO = 0.25
SALIDA = Path("LIMA-ESQUEMA-DIRECCIONAMIENTO-IA.xlsx")

# Segun el informe:
# Ventas 198, Administracion 100, Finanzas 41, Marketing 29,
# Logistica 25, Servidores 10, WiFi Clientes 18, WiFi Ejecutivos 31.
...
def validate_allocations(parent: ipaddress.IPv4Network, nets: list[ipaddress.IPv4Network]) -> None:
    assert all(net.subnet_of(parent) for net in nets), "Alguna VLAN queda fuera de Lima"
    for i, left in enumerate(nets):
        for right in nets[i + 1 :]:
            assert not left.overlaps(right), f"Solapamiento: {left} y {right}"


def allocate_vlsm() -> list[dict[str, str | int | ipaddress.IPv4Network]]:
    parent = ipaddress.ip_network(RED_LIMA)
    ...
    for unidad, actuales, vlan, vlan_id in UNIDADES:
        ...
        if int(net.broadcast_address) > end:
            raise ValueError(f"No hay espacio para {unidad}")
        rows.append(
            {
                "unidad": unidad,
                "actuales": actuales,
...
def build_sheet() -> str:
    ...
    rows = [
        row_xml(1, {1: ("Nombre de la Sede:", 1), 2: ("Lima - Sede Principal", 2)}),
        row_xml(2, {1: ("Direccion IP (subred):", 1), 2: (RED_LIMA, 2)}),
        row_xml(6, {3: ("Numero de Host por Subred", 3)}),
    ]
    ...
    rows.append(row_xml(7, {col: (text, 4) for col, text in headers.items()}))

    for idx, item in enumerate(data, start=8):
        rows.append(
            row_xml(
                idx,
                {
    ...
    rows.append(row_xml(total_row + 2, {1: ("Nota:", 1), 2: ("Nativa se calculo con 15 equipos actuales asumidos para gestion; ajustar si tu Packet Tracer usa otra cantidad. XYZW queda reservado.", 2)}))
    rows.append(row_xml(total_row + 3, {1: ("Validacion:", 1), 2: ("Todas las VLAN quedan dentro de 10.192.40.0/22 y no se solapan.", 2)}))
...
<worksheet xmlns="http://schemas.openxmlformats.org/spreadsheetml/2006/main"
 ...
 <mergeCells count="2"><mergeCell ref="C6:D6"/><mergeCell ref="B1:D1"/></mergeCells>
</worksheet>'''
...
<workbook xmlns="http://schemas.openxmlformats.org/spreadsheetml/2006/main"
 xmlns:r="http://schemas.openxmlformats.org/officeDocument/2006/relationships">
 <sheets><sheet name="Lima" sheetId="1" r:id="rId1"/></sheets>
</workbook>''',
...
<Relationships xmlns="http://schemas.openxmlformats.org/package/2006/relationships">
 ...
 <Relationship Id="rId2" Type="http://schemas.openxmlformats.org/officeDocument/2006/relationships/styles" Target="styles.xml"/>
    ...
    print("Validacion: todas las subredes quedan dentro de Lima y no se solapan.")
